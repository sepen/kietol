...
FIFO = os.path.expanduser("~/.kietol/fifo")
...
class Remote:
	...
	kietol, fifo, handle = None, None, 0
	...
	def _cmd_add(self, args):
		self.kietol.playlist.load(args)
	...
	def _cmd_play(self, args):
		try:  #attempt to play track at index
			self.kietol.playlist.play(args[0])
	...
	def _cmd_stop(self, args):
		self.kietol.playlist.stop(None, None)
	...
	def _cmd_prev(self, args):
		self.kietol.playlist.prev(None, None)
	
	#
	# Write the current target's title to fifo.
	#
	def _cmd_status(self, args):
		status = self.kietol.playlist.status()
