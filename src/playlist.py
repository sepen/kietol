...
PLAYLIST = os.path.expanduser("~/.kietol/playlist")
...
class Playlist:
	...
	def __init__(self, kietol):
		...
		self.continuous = kietol.prefs.getBool("continuous")
		self.random = kietol.prefs.getBool("random")
		self.repeat = kietol.prefs.getBool("repeat")
	...
	def menu(self, widget, event):
		...
		menu, time = self.kietol.menu, event.get_time()
	...
	def play(self, path, log=True, event=None):
		...
		self.kietol.mplayer.play(target)  #begin playing target
		self.kietol.window.set_title(title)
		return True
	...
	def stop(self, widget, event):
		...
		self.kietol.window.set_title(self.kietol.versionString)
		return True
	...
	def status(self):
		...
		if not self.current:
			return self.kietol.versionString
	...
	def add(self, target):
		...
		if self.loadcur >= self.loadcount:  #caught up, reorder list
			...
			self.kietol.control.setProgress(-1)
		else:  #update loading progress
			percent = 100 * self.loadcur / self.loadcount
			self.kietol.control.setProgress(percent)
